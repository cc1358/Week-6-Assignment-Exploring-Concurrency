...
func main() {
	...
	for size := 4; size <= numExplanatory; size++ {
		...
		for _, features := range combinations {
			...
			model := modelResult{Features: features, AIC: aic, MSE: mse}

			if aic < bestAIC {
				bestAIC = aic
			}
			bestModels = append(bestModels, model)
		}
	}

	fmt.Println("\nBest Models:")
	for _, m := range bestModels {
		fmt.Printf("Features: %v, AIC: %.4f, MSE: %.4f\n", m.Features, m.AIC, m.MSE)
	}
